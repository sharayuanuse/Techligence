...
const AddCourseModal = ({ isOpen, onClose, handleAddCourse }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const addCourse = async (formData) => {
      try {
        const res = await axios.post('http://localhost:8000/api/institute/courses/teacher' , formData)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backdropFilter: "blur(5px)" }}>
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md mx-auto p-6 relative z-10">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input type="file" name="thumbnail" onChange={handleFileChange} className="course-soft-input" accept="image/*"/>
